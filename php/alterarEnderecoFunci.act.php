...
if(isset($_POST['btenviar'])){
	...
	if(empty($_POST['cep'])){
		echo "<script language='javascript' type='text/javascript'>alert('Erro no campo de CEP ');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
	}else{

		$cepTrue = mysqli_query($con, "UPDATE `tbenderecofuncionario` SET cep = '$cep1', rua = '$rua1', 
		numero = '$numero11', bairro = '$bairro1', cidade = '$cidade1', estado = '$estado1' WHERE codFunci = '$codigoo'");//Atualiza endereco

		if($cepTrue){
			echo "<script language='javascript' type='text/javascript'>
			alert('Endereço alterado com Sucesso.');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
			mysqli_close($con);//Fecha consulta ao banco

		}else{
			echo "<script language='javascript' type='text/javascript'>
			alert('Erro ao alterar endereço.');window.location.href='../VerPerfilFunci.php?cod=$codigoo';</script>";
			mysqli_close($con);//Fecha consulta ao banco
		}
	}
