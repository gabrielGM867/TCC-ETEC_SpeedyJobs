...
<?php
	...
	if(isset($_GET['conversacom'])){
		...
		tbfuncionarios.nomeUsuario, tbfuncionarios.foto,
		tbusuarioschat.codigo AS 'codiChatFunci', tbusuarioschat.codUsuario, tbusuarioschat.grupo
		FROM `tbmensagens` 
		INNER JOIN tbusuarioschat ON tbusuarioschat.codigo = tbmensagens.id_de
		INNER JOIN tbfuncionarios ON tbfuncionarios.codigo = tbusuarioschat.codUsuario 
		WHERE 
		(id_de = '$online' AND id_para = '$id_conversa' AND `grupo` = 'c') 
		OR (id_de = '$id_conversa' AND id_para = '$online' AND `grupo` = 'f') 
