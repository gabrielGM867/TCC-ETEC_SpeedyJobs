...
if(isset($_POST['btenviar2'])){
	$nome1 = mysqli_real_escape_string($con, addslashes($_POST['nome2']));
	$nomeUsuario1 = mysqli_real_escape_string($con, addslashes($_POST['nomeUsuario2']));
	$email1 = mysqli_real_escape_string($con, addslashes($_POST['email2']));
	$profissao1 = mysqli_real_escape_string($con, addslashes($_POST['profissao']));
	$cpf1 = mysqli_real_escape_string($con, addslashes($_POST['cpf2']));
	$cep1 = mysqli_real_escape_string($con, addslashes($_POST['cep2']));
	$rua1 = mysqli_real_escape_string($con, addslashes($_POST['rua2']));
	$numero11 = mysqli_real_escape_string($con, addslashes($_POST['numero2']));
	$bairro1 = mysqli_real_escape_string($con, addslashes($_POST['bairro2']));
	$cidade1 = mysqli_real_escape_string($con, addslashes($_POST['cidade2']));
	$estado1 = mysqli_real_escape_string($con, addslashes($_POST['uf2']));
	$dataNascimento1 = mysqli_real_escape_string($con, addslashes($_POST['dataNascimento2']));
	$telefone1 = mysqli_real_escape_string($con, addslashes($_POST['telefone2']));
	$sexo1 = mysqli_real_escape_string($con, addslashes($_POST['sexo2']));
	$senha1 = mysqli_real_escape_string($con, addslashes($_POST['senha2']));
	$confirmSenha1 = mysqli_real_escape_string($con, addslashes($_POST['confirmSenha2']));
	...
	//Verifica se a senha tem mais de 50 dígitos
	...
	//Verifica se a profissão tem mais de 50 dígitos
	$resPro  = strlen ($profissao1);
	//-----------------------------------------------------------------------------------//
	...
	if(verficaCPF($cpf1) || 
		...
		if(empty($_POST['profissao'])){
			echo "<script language='javascript' type='text/javascript'>alert('Erro no campo de profissão ');window.location.href='../cadastro.php';</script>";
		}
		...
		if(empty($_POST['dataNascimento2'])){
		...
		if($resEmail > 50){
			echo "<script language='javascript' type='text/javascript'>alert('O email só pode ter no máximo 50 caracteres ');window.location.href='../cadastro.php';</script>";
		}

		if($resPro > 50){
			echo "<script language='javascript' type='text/javascript'>alert('A profissão só pode ter no máximo 50 caracteres ');window.location.href='../cadastro.php';</script>";
		}
		...
		$query3 = "SELECT * FROM `tbfuncionarios` WHERE nomeUsuario = '$nomeUsuario1'";
		$result3 = mysqli_query($con, $query3);
		$row3 = mysqli_num_rows($result3);
		$registro3 = false;

		if($row3>0){
			$registro3 = true; // Se o nome de usuário já existir o registro fica true
		}

		$query11 = "SELECT * FROM `tbclientes` WHERE nomeUsuario = '$nomeUsuario1'";
		...
		$query4 = "SELECT * FROM `tbfuncionarios` WHERE cpf = '$cpf1'";
						...
						if($registro3 == false and $registro11 == false){//Verifica se o nome de usuario já existe
							if($registro4 == false){//Verifica se o cpf já existe
								if($resgistro5 == true){//Verifica se a pessoa escreveu seu NOME COMPLETO
									...
									$inserir = "INSERT INTO tbfuncionarios";
									$inserir .= "(`nome`, `nomeUsuario`, `email`, `profissao`, `cpf`, `dataNascimento`, `telefone`, `sexo`, `senha`, 
									`classificacao`, `quantClass`, `foto`, `fotoCapa`, `statusEmail`)";
									$inserir .= "VALUES ";
									$inserir .= "('$nome1', '$nomeUsuario1', '$email1', '$profissao1', '$cpf1', '$dataNascimento1', '$telefone1', '$sexo1', '$senha1', '0', '0', '$foto_vazia', '0', '0')";
									...
									//Encontra maior codigo de funcionarios

									$maiorCodigo = "select max(codigo) as maiorCod from tbfuncionarios";
									...
									$inserirFunciNoChat = "INSERT INTO tbusuarioschat";
									$inserirFunciNoChat .= "(`codUsuario`, `grupo`)";
									$inserirFunciNoChat .= "VALUES ";
									$inserirFunciNoChat .= "('$str', 'f')";

									$operacapInserirFunciChat = mysqli_query ($con, $inserirFunciNoChat);

									if(!$operacapInserirFunciChat){
										die("Erro ao inserir usuario na tabela usuarios chat");

									}

									//Encontra maior codigo de funcionario no chat
									$maiorCodigo = "select max(codigo) as maiorCod from tbusuarioschat WHERE tbusuarioschat.grupo = 'f'";
									$resultado_maiorcodigo = mysqli_query ($con, $maiorCodigo);
									$row = mysqli_fetch_array($resultado_maiorcodigo);
									$row[0] = "";
									$row[1] = "";
									...
									$senhaTrue = mysqli_query($con, "UPDATE `tbfuncionarios` SET codtbUsuarios = '$codigoChatt' WHERE  codigo = '$str'");//Coloca codigo do chat na tabela de clientes
									...
									$inserir = "INSERT INTO tbtelefonesfuncionario";
									$inserir .= "(`codFunci`, `numero`, `tipo`)";
									$inserir .= "VALUES ";
									...
									$inserir = "INSERT INTO tbenderecofuncionario";
									$inserir .= "(`codFunci`, `cep`, `rua`, `numero`, `bairro`, `cidade`, `estado`)";
									$inserir .= "VALUES ";
									...
									<P>localhost/ProjetoSpeedyJobs/php/confirmaEmailFunci.act.php?cod=' . $str . '</P>';
