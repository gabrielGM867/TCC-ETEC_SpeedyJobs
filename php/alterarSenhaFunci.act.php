...
if(isset($_POST['btenviar'])){
	...
	if(empty($_POST['senhaAntiga'])){//Verifica se a senha e igual a confirmação de senha
		echo "<script language='javascript' type='text/javascript'>alert('Erro no campo de Senha Antiga ');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";

	}elseif(empty($_POST['senha'])){//Verifica se a senha e igual a confirmação de senha
		echo "<script language='javascript' type='text/javascript'>alert('Erro no campo de Senha ');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";

	}elseif($senha1 == $confirmSenha1){

		//Verifica se a SENHA ANTIGA é verdadeira coma a do Banco

		$query2 = "SELECT * FROM `tbfuncionarios` WHERE senha = '$senhaAntiga1'";
		...
		if($registro == true){
			...
			$senhaTrue = mysqli_query($con, "UPDATE `tbfuncionarios` SET senha = '$senhaNova' WHERE  codigo = '$codigoo'");//Atualiza senha


			if($senhaTrue){
				echo "<script language='javascript' type='text/javascript'>
				alert('Senha alterada com Sucesso.');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
				mysqli_close($con);//Fecha consulta ao banco

			}else{
				echo "<script language='javascript' type='text/javascript'>
				alert('Erro ao alterar senha.');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
				mysqli_close($con);//Fecha consulta ao banco
			}

		}else{
			echo "<script language='javascript' type='text/javascript'>
			alert('Senha antiga inválida');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
		}
	}
