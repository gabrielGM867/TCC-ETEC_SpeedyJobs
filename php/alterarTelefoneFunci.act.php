...
if(isset($_POST['btenviar'])){
	...
	if(empty($_POST['telefone'])){
		echo "<script language='javascript' type='text/javascript'>alert('Erro no campo de telefone ');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
	}else{

	
		$delTelefoneTrue = mysqli_query($con, "DELETE FROM `tbtelefonesfuncionario` WHERE codFunci = '$codigoo'");//Deleta telefone
		...
		if($operacao_inserir){
			...
			alert('Telefone alterado com Sucesso.');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
			mysqli_close($con);//Fecha consulta ao banco

		}else{
			echo "<script language='javascript' type='text/javascript'>
			alert('Erro ao alterar telefone.');window.location.href='../verPerfilFunci.php?cod=$codigoo';</script>";
			mysqli_close($con);//Fecha consulta ao banco
		}
