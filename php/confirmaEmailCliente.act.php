...
$busca = mysqli_query($con,"Select * from `tbclientes` where codigo = '$codigoo' ");
$usuario = mysqli_fetch_array($busca);

$_SESSION['status'] = $usuario['statusEmail'];//Pega o status de confirmação do email do cliente

if($_SESSION['status'] == 0){//Verifica se o cliente já confirmou ou não o e-mail	

	$statusTrue = mysqli_query($con, "UPDATE `tbclientes` SET statusEmail = '1' WHERE  codigo = '$codigoo'");//Confirma o e-mail
